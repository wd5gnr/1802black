...
PROB1B:	IRX\ LDX\ DEC SP	; get the unit number back again
	...
	CALL(TDEC16)		; type the size in megabytes
	INLMES("Mb ")		; ...
...
; 1802, and on the 1805/6 it's the "RLXA 8" (register load via X and advance)
				; instruction.
	;;  for emulation we are always an 1802 but it is handy to have
	;; SHOW CPU cause a break to the meta monitor
	;;  But not twice! So...
#ifdef BLACK1802
	.DB 68H
#else	
...
#ifdef BLACK1802				; the PF bit in status register C.
	LBR SHOCP9				; could use SIM key to pull millis
#else	
...
#else
	CMD(3, "CLS",      CLSCMD0) ; normal BIOS clearscreen
#endif
