...
int run(void)
{
  ...
  if ((reg[p] & 0xFF00) >= 0xF800)
  {
    ...
    if (rv)
      return rv;
#if 0  // enable this to see what unhandled BIOS calls a program is using      
    if ((reg[p]>=0xF800  && reg[p]<=0xF836) || (reg[p]>=0xFF00 && reg[p]<=0xFF81))
      Serial.printf("BIOS: %04x\r\n", reg[p]);
#endif      
  }
  // if we wind up here it was a BIOS call we don't know about so try to run it from ROM
#endif
  reg[p]++;
  I = inst >> 4;
  N = inst & 0xF;
...
// unfortunately, STG EPROM also uses it to test CPU type
// so if you get there, you'll need to move PC +2 and continue
#if MONITOR == 1
      if (!monactive)
        monitor();
      break;
